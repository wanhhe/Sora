...
int main()
{
    RendererWindow main_window(&camera, "Sora", WindowSize(window_width, window_height));
    // Create scene
    ...
    Shader* inverse_shader = new Shader(FileSystem::GetContentPath() / "Shader/framebuffer.vs", 
        FileSystem::GetContentPath() / "Shader/inverse_color.fs", true);

    Shader* blur_shader = new Shader(FileSystem::GetContentPath() / "Shader/framebuffer.vs", 
        FileSystem::GetContentPath() / "Shader/blur.fs", true);

    Shader* bloom_shader = new Shader(FileSystem::GetContentPath() / "Shader/framebuffer.vs", 
        FileSystem::GetContentPath() / "Shader/bloom.fs", true);

    Shader* ssao_shader = new Shader(FileSystem::GetContentPath() / "Shader/framebuffer.vs", 
        FileSystem::GetContentPath() / "Shader/SSAO.fs", true);

    Shader* hdr_shader = new Shader(FileSystem::GetContentPath() / "Shader/framebuffer.vs",
        FileSystem::GetContentPath() / "Shader/hdr.fs", true);
    ...
    hdr_shader->LoadShader();
    // raymarching_shader->LoadShader();
    npr_shader->LoadShader();
    ...
    ppm->AddPostProcess(ppm->CreatePostProcess<HDRProcess>(hdr_shader, "HDR", false));

    // Add a gamma correct post process
    ppm->AddPostProcess( ppm->CreatePostProcess<PostProcess>( gamma_correcting_shader, "Gamma correction", true ));
    ...
    while (!glfwWindowShouldClose(main_window.Window))
    {
        ...
        leftFocused = InputInfo::GetInstance()->mouse_button_left;
        if (leftFocused) {
            camera.ProcessLeftMouseMovement(InputInfo::GetInstance()->mouse_offset_x,
                InputInfo::GetInstance()->mouse_offset_y, deltaTime);
        }
        rightFocused = InputInfo::GetInstance()->mouse_button_right;
        if (rightFocused)
        {
            camera.ProcessRightMouseMovement(InputInfo::GetInstance()->mouse_offset_x,
