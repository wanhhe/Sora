...
Model::Model(string const& path, bool gamma) : gammaCorrection(gamma) { 
    ...
    const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace | aiProcess_PopulateArmatureData);
    // check for errors
    ...
    if (path.substr(path.find_last_of('.') + 1, path.size()) == "gltf") {
        ...
        std::vector<Mesh> res = LoadMeshes(gltf);

        for (unsigned int i = 0; i < res.size(); i++) {
            meshes[i]->SetSkin(res[i].GetVert());
            meshes[i]->AgainSetUp();
        }
        ...
        instance.mAnimatedPose = mSkeleton.GetRestPose();
        instance.mPosePalette.resize(mSkeleton.GetRestPose().Size());
        instance.load = true;
    }
}
...
Model::Model(std::filesystem::path path, bool gamma) : gammaCorrection(gamma) { 
    // read file via ASSIMP
    Assimp::Importer importer;
    const aiScene* scene = importer.ReadFile(path.string().c_str(), aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace | aiProcess_PopulateArmatureData);
    // check for errors
    if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) // if is Not Zero
    {
        RendererConsole::GetInstance()->AddError("[error] ASSIMP: %s", importer.GetErrorString());
    ...
    if (path.string().substr(path.string().find_last_of('.') + 1, path.string().size()) == "gltf") {
        cgltf_data* gltf = LoadGLTFFile(path.string().c_str());
        mSkeleton = LoadSkeleton(gltf);
        std::vector<Clip> clips = LoadAnimationClips(gltf);
        mClips.resize(clips.size());
        for (unsigned int i = 0; i < (unsigned int)clips.size(); i++) {
