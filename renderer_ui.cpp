...
void renderer_ui::SavePanel(RendererWindow* window, Scene* scene)
{
    if (!showSavePanel)
    {
        return;
    ...
    strcpy_s(save_path, scene_path.string().c_str());
    static std::string info = "";
    ImGui::InputText("Scene Path", save_path, IM_ARRAYSIZE(save_path));
    ImGui::SameLine();
    if (ImGui::Button("..."))
    {
        scene_path = FileSystem::GetContentPath();
        file_path = &scene_path;
        showFileBrowser = true;
    }
    static char save_name[128] = "Scene";
    ImGui::InputText("Scene Name", save_name, IM_ARRAYSIZE(save_name));
    if (ImGui::Button("Cancel"))
    {
        showSavePanel = false;
        strcpy_s(save_path, scene_path.string().c_str());
    ...
    if (ImGui::Button("Confirm"))
    {
        std::string path_s = scene_path.string();
        showSavePanel = false;
        scene->SaveScene(path_s+"/"+save_name+".scene");
        strcpy_s(save_path, scene_path.string().c_str());
    }
    ImGui::Text(info.c_str());
    ImGui::End();
...
void renderer_ui::LoadPanel(RendererWindow* window, Scene* scene)
{
    if (!showLoadPanel)
    {
        return;
    ...
    static char load_path[128];
    strcpy_s(load_path, scene_path.string().c_str());
    static std::string info = "";
    ImGui::InputText("Scene Path", load_path, IM_ARRAYSIZE(load_path));
    ImGui::SameLine();
    if (ImGui::Button("..."))
    {
        scene_path = FileSystem::GetContentPath();
...
void renderer_ui::mainUI(RendererWindow *window, Scene* scene)
{
    ...
    SavePanel(window, scene);
    LoadPanel(window, scene);
